...
def _assign_igroups(data, aux_vars):
    ...
    """
    data["conc_aux"] = data.apply(lambda r: str(list(map(lambda v: r[v], aux_vars))) if r["impute"] else "", axis=1)
...
_df4 = _df1 # Default behaviour, but user able to specify?  TODO: Check with FL
_df5 = _df2 # Default behaviour, but user able to specify?  TODO: Check with FL
_df6 = _df3 # Default behaviour, but user able to specify?  TODO: Check with FL
...
def _get_igroup_aux_var(data, aux_var):
    ...
    for i in range(1, 1+data["IGroup"].max()):
        out += data[["IGroup", aux_var]].drop_duplicates().query("IGroup == "+str(i))[aux_var].values.tolist()
    return out

def _calc_distances(data, aux_vars, dist_func, weights, threshold=None, custom_df=None):
    """
               data (pd.DataFrame): The dataset undergoing imputation
               aux_vars (str list): The names of the chosen auxiliary variables
                   dist_func (int): Which of the six standard distance functions
                                    to use
      weights (str * numeric dict): Dictionary with auxiliary variable names as
                                    keys and chosen weights as values
               threshold (numeric): [Optional] A threshold value for the
                                    distance function, if required
    custom_df ('a * 'a -> numeric): [Optional] A custom function accepting two
                                    inputs (the IGroup's value and the current
                                    record's value) and returning a numeric
                                    distance value
    ...
    if dist_func == 3: assert threshold
    # If custom distance function required, check that it is provided
    if dist_func >= 4: assert custom_df
    # Check that all of the auxiliary variables have weights specified
    assert all(map(lambda v: v in aux_vars, weights.keys()))
    # Calculate the distances
    dist_func = [_df1, _df2, _df3, custom_df, custom_df, custom_df][dist_func-1]
    igroup_aux_vars = []
    ...
    data["dists_temp"] = data.apply(lambda r: list(map(lambda x: {k: weights[k] * dist_func(x[k], r[k]) for k in aux_vars}, igroup_aux_vars)) if not(r["impute"]) else [], axis=1)
    data["distances"] = data.apply(lambda r: list(map(lambda d: reduce(add, list(d.values()), 0), r["dists_temp"])), axis=1)
    del data["dists_temp"]
...
def impute(data, impute_var, aux_vars, weights, dist_func, min_margin, overwrite, col_name=None):
    # data: dataframe
    # impute_var: string
...
def t1(): _add_impute_col(test_data,test_imp_var)
def t2(): _assign_igroups(test_data,test_aux_vars)
def t3(): _calc_distances(test_data,test_aux_vars,1,test_weights)
