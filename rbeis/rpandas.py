...
def r_check_missing_auxvars(data, aux_vars):
    """
...
def r_add_impute_col(data, imp_var):
    """
...
def r_assign_igroups(data, aux_var_names):
    """
...
def r_get_igroup_aux_var(data, aux_var_name):
    """
...
def r_calc_distances(data, aux_vars):
    """
    ...
    vars_vals = list(
        zip(
            aux_vars.keys(),
            map(lambda k: r_get_igroup_aux_var(data, k), aux_vars.keys()),
        ))
    for i in range(1 + data["_IGroup"].max()):
...
def r_calc_donors(data, ratio=None):
    """
...
def r_get_freq_dist(data, imp_var, possible_vals, igroup):
    """
    ...
    ```
    """
    pool = data[r_get_donors(data, igroup)][imp_var].values.tolist()
    return list(
        map(
...
def r_freq_to_exp(data, freq_dist, igroup):
    """
...
def r_impute_igroup(data, exp_dist, possible_vals, igroup):
    """
...
#        raise RBEISInputException("The column to undergo imputation contains values that are not included in possible_vals")
    ...
    r_check_missing_auxvars(data, aux_vars)
    r_add_impute_col(data, imp_var)
    r_assign_igroups(data, aux_vars.keys())
    r_calc_distances(data, aux_vars)
    r_calc_donors(data, ratio=ratio)
    imputed_vals = list(
        map(
            lambda i: r_impute_igroup(
                data,
                r_freq_to_exp(
                    data, r_get_freq_dist(data, imp_var, possible_vals, i), i),
                possible_vals,
                i,
            ),
            range(1 + data["_IGroup"].max()),
